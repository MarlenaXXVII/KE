...
(function() {
    const COMPONENT_ID = 'recommendation_earlier_viewed';
    const RECOMMENDATION_ID = '65f8020a938f32f74e5c2926';
    const HEADLINE = "Deze producten heb je eerder bekeken"
    // this is element that we are using to inject our carousel after
    const ELEMENT_TO_INJECT_AFTER = document.querySelector('.K1-advice-slider');
    ...
    const createCarousel = (recievedProductData) => {
        ...
        const validateProduct = (product) => {
            ...
            if(!product.image || product.image==null || product.image == 'https://www.kamera-express.nl/') {
                valid = false;
            }
        ...
        if(!productData || productData.length < 4 || !ELEMENT_TO_INJECT_AFTER) {
            return;
        }
        ...
        const getItemPartial = (item) => {
            ...
            const itemTemplate = `<a class=ab-rcmd-item data-id="${item.item_id}" href="${item.url}"><div class=ab-rcmd-item__img><img alt=""data-v-0aa41950=""height=200 loading=lazy src="${item.image}" width=200></div><div class=ab-rcmd-item__name><p>${item.title}</div><div class="ab-rating-wrapper"><div class="ab-rating"><span style="width: ${productRating}%;"></span></div><div class="ab-rating-votes">(${item.amount_of_reviews})</div></div><div class=ab-rcmd-item__price><p class=current-price><span>${priceMain}</span> <span class=decimal>${priceReminder}</span></div></a>`
            return itemTemplate;
        }
        ...
        const getCarouselPartial = (data) => {
            ...
            const carouselTemplate = `
                <div class="K1-container K1-mb-container" id="${COMPONENT_ID}">
                    <div class="K1-slider-title">
                        <h2>${HEADLINE}</h2>
                    </div>            
                    <div class="ab-rcmd-carousel ab-rcmd-noprev">
                        ...
                        <div class="ab-rcmd-carousel-buttons">
                            <span class="ab-rcmd-carousel-prev"><span class="K1-icon-arrow-left"></span></span>
                            <span class="ab-rcmd-carousel-next"><span class="K1-icon-arrow-right"></span></span>
                        </div>
                    </div>
                </div>
            `
        ...
        const getMaxOffset = () => {
            ...
            const maxOffset = itemsToScroll*Math.floor(productData.length/itemsToScroll);
            return maxOffset;
        }
        ...
        if(carouselTemplate) {
            ...
            ELEMENT_TO_INJECT_AFTER.after(carouselHTML);
