...
function setProductRecommendations(options) {
    ...
    const createCarousel = (recievedProductData) => {
        ...
        const getItemPartial = (item) => {
            ...
            const itemTemplate = `<a class=ab-rcmd-item data-id="${item.item_id}" href="${item.url}"><div class=ab-rcmd-item__img><img alt=""data-v-0aa41950=""height=200 loading=lazy src="${item.image}" width=200></div><div class=ab-rcmd-item__name><p>${item.title}</div><div class="ab-rating-wrapper ${ratingClass}"><div class="ab-rating"><span style="width: ${productRating}%;"></span></div><div class="ab-rating-votes">(${item.amount_of_reviews})</div></div><div class=ab-rcmd-item__price><p class=current-price><span>${priceMain}</span> <span class=decimal>${priceReminder}</span></div></a>`
            return itemTemplate;
        }
...
function pdp_personalizedblocksq4() {
	if(!document.querySelector('.image-gallery-buy-section')) {
	    return;
	}
    const pdp_personalizedblocksq4 = document.createElement('style');
    pdp_personalizedblocksq4.textContent = '.K1-products-slider {display: none;}'
    document.querySelector('head').append(pdp_personalizedblocksq4);


    setProductRecommendations({
        componentId: 'pdp_personalizedblocksq4',
        componentClasses: '',
        recommendationID: '671a25b879b7858834a42988',
        headline: 'Diese Produkte hast du dir bereits angesehen',
        styles: '',
        placement: {
            selector: '.slug-page-content',
            action: 'append'
        }
    });
}

pdp_personalizedblocksq4();