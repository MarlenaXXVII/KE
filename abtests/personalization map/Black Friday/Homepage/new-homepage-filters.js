...
function setProductRecommendations(options) {
    ...
    const createCarousel = (recievedProductData) => {
        ...
        const validateProduct = (product, componentID) => {
            let valid = true;
            ...
            if (componentID === "hp_personalizedblocksq4_1") {
                if (product.text_wobbler.includes("Profotonet") || product.text_wobbler.includes("Giftbox") || 
                    product.text_wobbler.includes("Nieuw") || product.text_wobbler.includes("garantie") || 
                    product.text_wobbler.includes("Opruiming") || product.text_wobbler.includes("Premium") || 
                    product.text_wobbler === "") {
                    valid = false;
                }
            }
        ...
        recievedProductData.map((product) => {
            if (validateProduct(product, COMPONENT_ID)) {
